    ...
    def updateView(self):
        ...
        if not self.modApp.globalModelView:
            ...
            if  np.float64(y.max()) >  np.float64(y.max()) and  np.float64(y.min()) <  np.float64(y.min()):
                ...
                #currax.plot(val_node_exp, y, 'ro', label="Local Model")
                currax.plot(val_node_exp[[0, 1, 8]], y[[0, 1, 8]], 'ro', label="Local Model")
                currax.plot(val_node_exp[[2, 3, 9]], y[[2, 3, 9]], 'ro', label="Local Model")
                currax.plot(val_node_exp[[4, 5, 10]], y[[4, 5, 10]], 'ro', label="Local Model")
                currax.plot(val_node_exp[[6, 7, 11]], y[[6, 7, 11]], 'ro', label="Local Model")
                currax.plot(val_node_exp, z, 'bo', label="Global Model")
                currax.plot([mn-inter, mx + inter], [mn-inter, mx + inter], 'r-')
