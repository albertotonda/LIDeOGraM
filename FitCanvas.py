...
# TODO Crée la courbe correspondant à l'équation sélectionnée
class FitCanvas(FigureCanvas):
    ...
    def updateView(self):
        ...
        if not self.modApp.globalModelView:
            ...
            currax.plot(val_node_exp[[2, 3, 9]], y[[2, 3, 9]], 'bo', label="22C 6h")
            currax.plot(val_node_exp[[4, 5, 10]], y[[4, 5, 10]], 'yo', label="30C 0h")
            currax.plot(val_node_exp[[6, 7, 11]], y[[6, 7, 11]], 'go', label="30C 6h")
            ...
            if  np.float64(y.max()) >  np.float64(y.max()) and  np.float64(y.min()) <  np.float64(y.min()):
                mx = np.maximum(val_node_exp.max(), np.float64(y.max()))
                mn = np.minimum(val_node_exp.min(), np.float64(y.min()))
                inter = (val_node_exp.max() - val_node_exp.min()) * 0.1
                #currax.plot(val_node_exp, y, 'ro', label="Local Model")
