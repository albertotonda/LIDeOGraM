...
class Network:
    ...
    def higlight(self, new_node: str, old_node: str = None):
        self.G.clear()
        if old_node:
            self.nodeColor[(self.modApp.varnames.tolist()).index(old_node)] = self.modApp.old_color

        self.modApp.old_color = self.nodeColor[(self.modApp.varnames.tolist()).index(new_node)]
        self.nodeColor[(self.modApp.varnames.tolist()).index(new_node)] = (1.0,0,0)
        nx.draw_networkx_nodes(self.G, self.pos, nodelist=self.modApp.varnames.tolist(), node_color=self.nodeColor, with_labels=False, ax=self.axes)
        nx.draw_networkx_labels(self.G, self.lpos, self.labels, ax=self.axes)
        nx.draw_networkx_edges(self.G,self.pos,**self.edges)
        self.updateView()
    ...
    def updateView(self):
        ...
        self.edges =  {"edgelist":edgelist_inOrder, "edge_color":self.edgeColor, "ax":self.axes}
