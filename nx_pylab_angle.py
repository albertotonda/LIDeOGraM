    ...
    if G.is_directed() and arrows:
        ...
        for src, dst in edge_pos:
            ...
            if dy == 0:  # horizontal edge
                ...
                xa = d*numpy.cos(theta)+x1
                ya = d*numpy.sin(theta)+y1
            ...
            ya1=numpy.sin(edgeAngle1)*0.07+y0r
            xa2=numpy.cos(edgeAngle2)*0.07+x0r
            ya2=numpy.sin(edgeAngle2)*0.07+y0r
            dr1 = numpy.sqrt(float((xa1-x2) ** 2 + (ya1-y2) ** 2))
            dr2 = numpy.sqrt(float((xa2-x2) ** 2 + (ya2-y2) ** 2))

            a_pos.append(((xa1, ya1), (x2, y2)))
            a_pos.append(((xa2, ya2), (x2, y2)))
...
def draw_networkx_labels_angle(G, pos,
                         labels=None,
                         label_pos=0.5,
                         font_size=12,
                         font_color='k',
    ...
    for n, label in labels.items():
        ...
        # set optional alignment
        horizontalalignment = kwds.get('horizontalalignment', 'left')
        verticalalignment = kwds.get('verticalalignment',  'bottom')

        #t = ax.text(x, y,
        rotateRad=(rotate/360)*numpy.pi
        #x+=numpy.cos(rotateRad)*30*len(label)/2
        y-=numpy.sin(rotateRad)*0.005*len(label)/2
        t = ax.text(x, y,
                    label,
                    size=font_size,
                    color=font_color,
